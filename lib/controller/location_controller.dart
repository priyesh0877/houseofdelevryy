...
import 'package:houseofdelevryy/data/model/response/place_details_model.dart';
import 'package:houseofdelevryy/data/model/response/prediction_model.dart';
...
class LocationController extends GetxController implements GetxService {
  ...
  Future<ZoneResponseModel> getZone(String lat, String long, bool markerLoad, {bool updateInAddress = false}) async {
    ...
    if(response.statusCode == 200) {
      ...
      _inZone = false;
      _responseModel = ZoneResponseModel(false, response.statusText.toString(), [], []);
    }
    if(markerLoad) {
      _loading = false;
    }else {
  ...
  Future<ResponseModel> addAddress(AddressModel addressModel, bool fromCheckout, int restaurantZoneId) async {
    ...
    if (response.statusCode == 200) {
      ...
      responseModel = ResponseModel(false, response.statusText == 'Out of coverage!' ? 'service_not_available_in_this_area'.tr : response.statusText.toString());
    }
    update();
