...
class ProductController extends GetxController implements GetxService {
  ...
  Future<void> getPopularProductList(bool reload, String type, bool notify) async {
    _popularType = type;
    if(reload) {
      _popularProductList = _popularProductList;
    }
    if(notify) {
      update();
    }
  ...
  Future<void> getReviewedProductList(bool reload, String type, bool notify) async {
    _reviewedType = type;
    if(reload) {
      _reviewedProductList = _reviewedProductList;
    }
    if(notify) {
      update();
    }
  ...
  int setExistInCart(Product product, {bool notify = true}) {
    ...
    _cartIndex = Get.find<CartController>().isExistInCart(product.id, variationType, false, _cartIndex);
    if(_cartIndex != -1) {
  ...
  bool isAvailable(Product product) {
    return DateConverter.isAvailable(product.availableTimeStarts, product.availableTimeEnds, time: DateTime.now());
  }
