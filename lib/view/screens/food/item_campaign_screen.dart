...
class _ItemCampaignScreenState extends State<ItemCampaignScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: CustomAppBar(title: 'campaigns'.tr, onBackPressed: (){},),
      body: Scrollbar(child: SingleChildScrollView(child: Center(child: SizedBox(
        ...
        child: GetBuilder<CampaignController>(builder: (campController) {
          return ProductView(
            isRestaurant: false, products: campController.itemCampaignList, restaurants: List.empty(),
            isCampaign: true, noDataText: 'no_campaign_found'.tr, type: '', onVegFilterTap: (String type) {  },
          );
        }),
      )))),
    );
